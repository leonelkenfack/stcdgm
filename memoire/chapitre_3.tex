...
\noindent III.4 \ Encodeur intelligible et réseau causal récurrent \dotfill \pageref{sec:oracle-encoder}\\
\noindent III.5 \ Apprentissage du DAG par DAGMA \dotfill \pageref{sec:oracle-dagma}\\
\noindent III.6 \ Décodeur graphe-à-grille \dotfill \pageref{sec:oracle-decoder}\\
\noindent III.7 \ Interface inter-étages, propriété (O3) et diffusion EDM \dotfill \pageref{sec:oracle-o3}\\
\noindent III.8 \ Pertes et entraînement \dotfill \pageref{sec:oracle-pertes}\\
\noindent III.9 \ Synthèse et traçabilité \dotfill \pageref{sec:oracle-synthese}
\end{sommairebox}
...
\begin{itemize}[leftmargin=1.2em, itemsep=2pt]
    ...
    \item \textbf{O3} causalité architecturale --- dépendance non-triviale de $\muHR$ à $\Adag$, signature contrastive vis-à-vis de \CorrDiff{} ;
...
\end{itemize}
Décomposition deux étages et diffusion EDM adressent O1--O2 ; RCN et contrainte DAGMA adressent O3 et O6 ; la structure causale adresse O4 ; le pré-calcul et la diffusion sur résidu de faible amplitude adressent O5.
...
\begin{itemize}[leftmargin=1.2em, itemsep=2pt]
    \item \textbf{Matériel} --- un benchmark sur CPU 48 cœurs montre qu'un GAT à 4 têtes multiplie le temps par batch par $\sim$5 et la mémoire d'activation par $\sim$3,5, rendant Stage~1 ($\sim$140 batches/époque, 75 époques) infaisable.
    \item \textbf{Théorique} --- SAGEConv moyenne localement (filtre passe-bas) et peut lisser les anomalies extrêmes ; cette limite est compensée globalement par $\Adag$, qui surimpose une pondération causale asymétrique entre variables. La synergie est explicite : SAGEConv assume le lissage spatial local, $\Adag$ porte la hiérarchie causale globale --- là où un GAT paierait l'attention deux fois.
\end{itemize}

\subsection{Réseau causal récurrent --- principe : SCM dynamique et mise à jour temporelle}
\label{sec:oracle-rcn}

Le \emph{Recurrent Causal Network} (RCN) forme le cœur méthodologique d'\Oracle{}. À chaque pas de temps, il combine deux briques : un \emph{modèle causal structurel} (SCM) qui prédit l'évolution de l'état caché à partir du DAG appris, et une \emph{mise à jour récurrente} (GRU) qui fusionne cette prédiction théorique avec l'observation courante. Le SCM joue le rôle du \emph{plan de vol} théorique, la GRU celui de la \emph{mise à jour aux instruments}. L'originalité d'\Oracle{} tient à ceci : \emph{le SCM est le moteur récurrent lui-même}. La transition $\Ht \to H_{t+1}$ est une assignation structurelle causale, pas un post-traitement appliqué à un état caché produit par ailleurs.
...
Apprendre conjointement la structure d'un DAG et les fonctions structurelles est combinatoire (le nombre de DAG à $N$ nœuds croît super-exponentiellement). Zheng et~al.~\cite{zheng_notears_2018} ont reformulé l'acyclicité comme contrainte différentiable $h_{\mathrm{NOTEARS}}(A) = \mathrm{tr}(e^{A \odot A}) - N$, coûteuse numériquement. Bello, Aragam et Ravikumar~\cite{bello_dagma_2022} ont proposé une caractérisation par log-déterminant d'une matrice M-positive :
\begin{equation}
  h_{\mathrm{DAGMA}}(A;\, s) \;=\; -\log \det\!\left( s\,I - A \odot A \right) + N \log s,
  \label{eq:dagma}
\end{equation}
où $s > N \cdot \max_{i,j} |A_{i,j}|$ garantit la M-positivité. $h_{\mathrm{DAGMA}}$ s'annule ssi $A$ représente un DAG, ne requiert qu'une décomposition LU, et son gradient se déduit de l'inverse de $sI - A \odot A$. Dans \Oracle{}, cette pénalité est combinée à une sparsité $L_1$ encourageant une matrice creuse et auditable :
\begin{equation}
...
Le décodeur transforme l'état caché final $\HT \in \mathbb{R}^{B \times N \times d}$ en une carte $\muHR \in \mathbb{R}^{B \times 1 \times H_{\HR} \times W_{\HR}}$ en deux étapes : attention croisée entre des requêtes définies sur une grille intermédiaire $43 \times 45$ (double de la résolution LR) et l'ensemble des nœuds du graphe (clés/valeurs), suivie d'un sur-échantillonnage bilinéaire et d'un raffinement convolutif vers la résolution HR. L'attention croisée multi-têtes calcule
\begin{equation}
...
\subsubsection{Pourquoi le résidu plutôt que le champ complet}
\label{ssec:oracle-residu-justif}

Modéliser $\delta$ plutôt que $\xHR$ réduit l'amplitude de la cible : $\xHR$ couvre plusieurs ordres de grandeur, $\delta$ est centré autour de zéro avec un écart-type de l'ordre de $\sigma_{\mathrm{data}}$ après log-transformation. La calibration de $\sigma_{\mathrm{data}}$ retrouve un sens physique et la variance prédite s'aligne mieux sur la variance observée des résiduels, réduisant la sub-dispersion typique des modèles entraînés sur champ complet (O2). L'interface inter-étages joue le rôle d'un \emph{pacte architectural} : le Stage 1 délivre $\muHR$ comme seule information conditionnelle, le Stage 2 la reçoit par concaténation sans voie d'attention parallèle, et l'ablation $\Adag \leftarrow \mathbf{0}$ n'a d'effet sur la sortie que si elle modifie effectivement $\muHR$ --- ce qui requiert simultanément que le Stage 1 utilise $\Adag$ et que le Stage 2 dépende de $\muHR$.
...
\subsection{Pertes et procédure d'entraînement}
...
\end{equation}
La procédure se déroule en deux phases. Le Stage 1 est entraîné par AdamW jusqu'à convergence ; ses paramètres sont alors gelés et on pré-calcule $\muHR^{(i)}$, $\mathrm{baseline}^{(i)}$ et $\sigma_{\mathrm{data}}$ empirique. Le critère (O3) joue le rôle de \emph{gate} : si $\Delta_{O3}/\mathrm{MSE} < \tau$, le Stage 1 est ré-entraîné. Le Stage 2 minimise ensuite exclusivement $\LEDM$ (équation~\ref{eq:edm-loss}) sur les résiduels pré-calculés. Trois éléments du protocole : l'EMA (décroissance $0{,}9999$) stabilise l'échantillonnage, le \emph{conditioning dropout} ($p \approx 0{,}13$) prépare la classifier-free guidance, et \texttt{cfg\_scale} est calibré au chapitre suivant.
...
Plusieurs alternatives ont été envisagées puis écartées : les \emph{Diffusion Transformers} (coût supérieur au U-Net à la résolution $172\times 179$, apport empirique non démontré pour le downscaling à cette échelle) ; le \emph{flow matching} et les \emph{consistency models} (maturité limitée sur cette application, perspective de continuation signalée en conclusion) ; l'injection des drivers statiques comme canaux supplémentaires du U-Net Stage 2 (écartée pour préserver le passage architectural par le DAG, condition de la validité de O3) ; la cross-attention pour le conditionnement Stage 2 (écartée au profit de la concaténation, cf. section~\ref{sec:oracle-edm}).
